...
$.ender({
  enhance: function() {
    if (history.pushState) {
      return $('a', this).union(this.matching('a')).each(function() {
        if ($(this).is('a[data-precache != "true"]:local-link(0)')) {
          $(this).bind('click', function(event) {
            history.pushState(true, '', this.pathname);
            soma.load(this.pathname);
            event.stop();
          });
        } else if ($(this).is('a[data-precache = "true"]:local-link(0)')) {
...
soma.Context = (function(_super) {
  ...
  function Context(path, lazy) {
    ...
    this.on('render', function() {
      var nextView;
      $.enhance();
      nextView = function(err) {
        var async, data, url, _ref;
        if (err) {
          throw err;
        }
        ...
        _ref = _this.views.shift(), url = _ref.url, data = _ref.data, async = _ref.async;
        if (async) {
          return _this.loadChunk(url, data, nextView);
        } else {
          _this.loadChunk(url, data);
          return nextView();
        }
      };
      nextView();
